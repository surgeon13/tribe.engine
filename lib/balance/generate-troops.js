...
import { CAV_REFS, INF_REFS, ROLE_SHAPES } from "./troop-roles.js";
...
const CAV_TIER = Object.freeze({
  cav_t1: { atk: 0.82, def: 0.88, speed: 1.12, carry: 1.12 },
  cav_t2: { atk: 1, def: 1, speed: 1, carry: 0.95 },
  cav_t3: { atk: 1.14, def: 1.1, speed: 0.95, carry: 0.85 },
});
...
export function buildTribeTable(tribeId, calibration = {}) {
  ...
  for (const ref of ALL_REFS) {
    const isCombat = COMBAT_REFS.includes(ref);
    ...
    const priced =
      ref === "chief" || ref === "settler"
        ? priceExpansion(ref, stats, spec)
        : priceUnit(ref, stats, spec);
    troops[ref] = { role: isCombat ? spec.slotRoles[ref] : ref, stats, ...priced };
  }
  return troops;
}
