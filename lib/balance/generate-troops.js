...
/**
 * Light and fast, then heavier and slower. The last stable unlock is the best
 * horse a tribe has, the way an Equites Caesaris or a Haeduan is.
 */
const CAV_TIER = Object.freeze({
  cav_t1: { atk: 0.78, def: 0.84, speed: 1.12, carry: 1.12 },
  cav_t2: { atk: 0.96, def: 0.96, speed: 1, carry: 0.95 },
...
function shapeStats(ref, roleId, spec, calibration) {
  ...
  return {
    attack: ref === "scout" ? 0 : clamp(Math.round(attack), 1, 320),
    defenseInfantry: clamp(Math.round(defInf), 5, 260),
    defenseCavalry: clamp(Math.round(defCav), 5, 260),
...
function liftCenterpiece(stats, spec) {
  ...
  const { stats: lifted } = liftToCenterpiece(stats.cav_t3, rival, spec.centerpiece ?? CENTERPIECE_LEAD);
  return {
    ...lifted,
    attack: lifted.attack ? clamp(lifted.attack, 1, 320) : 0,
    defenseInfantry: clamp(lifted.defenseInfantry, 5, 260),
    defenseCavalry: clamp(lifted.defenseCavalry, 5, 260),
  };
}
