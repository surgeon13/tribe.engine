...
import { STAT_STEP, quantizeStats } from "./quantize.js";
import {
  CAV_REFS,
  CENTERPIECE_AXIS,
  CENTERPIECE_LEAD,
...
/**
 * CAV_TIER means the two light horses to split the utility crowns: the first is
 * the courier, the second the pack horse. It does not get them, because the tier
 * modifier multiplies a role shape that already leans the other way — a Kihei is
 * 110 carry cut by 0.92, a Samurai 85 carry lifted by 1.24, and the two land four
 * percent apart, which the five-point grid then rounds into a dead heat. Two
 * horses with identical speed and carry are one horse you can build twice, so the
 * split is settled here rather than left to arithmetic: one clear step each way,
 * which is the smallest gap a player can actually read off the table.
 *
 * Only speed and carry move. Both are profile rather than size — carry counts for
 * 0.02 of the combat index and speed for 0.1 — so the centrepiece keeps its lead
 * and the tribe keeps its price.
 *
 * @param {Record<string, Stats>} stats every shaped combat slot, mutated in place
 */
function splitLightHorseCrowns(stats) {
  const [t1, t2, t3] = [stats.cav_t1, stats.cav_t2, stats.cav_t3];
  if (!t1 || !t2 || !t3) return;

  t1.speed = Math.max(t1.speed, t2.speed + 1, t3.speed + 1);
  t2.carry = Math.max(t2.carry, t1.carry + STAT_STEP, t3.carry + STAT_STEP);
}
