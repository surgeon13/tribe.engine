...
/**
 * The offense-only half of combatIndex. Blending attack and defense into one
 * number hides tribes whose power is lopsided rather than average — a tribe
 * can score as "roughly average power" while actually being far above on one
 * axis and far below on the other (Huns are the clearest case: +21% offense,
 * -25% defense, and a blended power of 0.98). Use this alongside
 * defenseIndex when the shape of a tribe's power matters, not just its total.
 * @param {Stats} stats
 * @returns {number}
 */
export function offenseIndex(stats) {
  return Number(stats.attack) || 0;
}

/**
 * The defense-only half of combatIndex — see offenseIndex.
 * @param {Stats} stats
 * @returns {number}
 */
export function defenseIndex(stats) {
  return 0.5 * ((Number(stats.defenseInfantry) || 0) + (Number(stats.defenseCavalry) || 0));
}
 ...
 * @property {number} atk Median attack for the slot (see offenseIndex).
 * @property {number} def Median defense (average of vs. infantry and vs.
 *   cavalry) for the slot (see defenseIndex).
