...
import { quantizeCost, quantizeStats, settleCenterpiece } from "./quantize.js";
...
export const ROLE_SHAPES = Object.freeze({
  line: {
    notes:
      "General line infantry — mild hybrid, Legionnaire / Hoplite family. Owns the " +
      "foot carry crown: a hybrid leans on nothing by definition, so if it does not " +
      "haul the most it is squeezed out of every column by the specialists beside it — " +
      "attack goes to off_inf, anti-cavalry to def_cav — and becomes a unit with no job.",
    stats: { attack: 38, defenseInfantry: 38, defenseCavalry: 45, speed: 6, carry: 60 },
    roleCostMul: 1,
  },
  off_inf: {
    notes: "Offensive infantry — Imperian / Axeman / Swordsman. Owns attack, so it yields carry to the line.",
    stats: { attack: 68, defenseInfantry: 38, defenseCavalry: 28, speed: 7, carry: 45 },
    roleCostMul: 1.05,
  },
  def_inf: {
...
/**
 * Ceilings for the combat stats, kept equal on purpose.
 *
 * A point of defence counts half a point of attack in the combat index, so a
 * wall needs roughly twice the raw number to weigh what a hammer weighs. While
 * defence was capped lower than attack, every defensive centerpiece in the
 * game ran into the ceiling and stopped: five of them sat on exactly the same
 * anti-infantry number, and their tribes quietly missed the lead their dial
 * asked for.
 */
export const STAT_CEILING = Object.freeze({ attack: 320, defense: 320 });
...
export function liftToCenterpiece(unit, rivalIndex, lead = CENTERPIECE_LEAD, axis = "both") {
  ...
  // The unit is put back on the five-point grid on the way out of both paths,
  // and topped up if the rounding cost it the lead. See lib/balance/quantize.js.
  const settle = (stats) => settleCenterpiece(stats, rivalIndex * lead, axis, STAT_CEILING.defense);
  if (!(power > 0) || target <= power) return { stats: settle(unit), factor: 1 };
  ...
  const [defenseInfantry, defenseCavalry] = splitDefense(
    unit.defenseInfantry * defMul,
    unit.defenseCavalry * defMul
  );

  return {
    stats: settle({
      ...unit,
      attack: attack ? Math.min(Math.round(attack * atkMul), STAT_CEILING.attack) : 0,
      defenseInfantry,
      defenseCavalry,
    }),
    factor: target / power,
...
/**
 * Place a defensive total across the two defence stats, spilling whatever will
 * not fit under the ceiling into the other side.
 *
 * The role shapes are lopsided by design — a Paladin stops infantry far better
 * than cavalry — and multiplying a lopsided pair pushes one side into the
 * ceiling long before the other. Clipping there would throw the excess away
 * and leave the unit weighing less than it was asked to, so it moves to
 * whichever side still has room instead.
 *
 * @param {number} infantry
 * @param {number} cavalry
 * @returns {[number, number]}
 */
function splitDefense(infantry, cavalry) {
  const cap = STAT_CEILING.defense;
  let inf = infantry;
  let cav = cavalry;
  if (inf > cap) {
    cav += inf - cap;
  ...
  if (cav > cap) {
    inf = Math.min(cap, inf + (cav - cap));
    cav = cap;
  }
  return [Math.round(inf), Math.round(cav)];
}
...
export function shapeTroopForRole(ref, base, roleId, scale) {
  ...
  return { stats, cost: quantizeCost(cost), cropUpkeep: base.cropUpkeep, roleId };
}
