...
/**
 * Grow the tier-3 horse until it leads the roster, keeping the shape its role
 * gave it: attack and both defenses move together, so a defensive centerpiece
 * gets harder to kill rather than turning into a hammer. Speed and carry are
 * profile rather than size and stay put.
 *
 ...
 * @param {number} [lead]
 * @returns {{ stats: Stats, factor: number }}
 */
export function liftToCenterpiece(unit, rivalIndex, lead = CENTERPIECE_LEAD) {
  const fixed = 0.1 * (unit.speed || 0) + 0.02 * (unit.carry || 0);
  const power = (unit.attack || 0) + 0.5 * ((unit.defenseInfantry || 0) + (unit.defenseCavalry || 0));
  const factor = power > 0 ? (rivalIndex * lead - fixed) / power : 1;
  if (!(factor > 1)) return { stats: unit, factor: 1 };
  return {
    stats: {
      ...unit,
      attack: unit.attack ? Math.round(unit.attack * factor) : 0,
      defenseInfantry: Math.round(unit.defenseInfantry * factor),
      defenseCavalry: Math.round(unit.defenseCavalry * factor),
    },
    factor,
  };
}
