...
export function computeFairness(troops) {
  ...
  for (const ref of SCORED_REFS) {
    ...
    const stats = u.stats || {};
    const unitCi = combatIndex(stats);

    ci += unitCi;
    atk += offenseIndex(stats);
    def += defenseIndex(stats);
    crop += u.cropUpkeep || 0;
    res += totalCost(u.cost);
    if (u.trainSeconds != null) time += u.trainSeconds;

    anchorCi += anchor.ci;
    anchorAtk += anchor.atk;
    anchorDef += anchor.def;
  ...
  return {
    // >1 means more raw combat index than the anchor roster. Blending
    // offense and defense into one number hides a lopsided tribe — see
    // offensePower/defensePower for the two halves separately.
    power: anchorCi ? ci / anchorCi : 0,
    offensePower: anchorAtk ? atk / anchorAtk : 0,
    defensePower: anchorDef ? def / anchorDef : 0,
