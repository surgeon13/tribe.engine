...
export async function handleApiRequest(req) {
  ...
  if (pathname === "/tribes" && method === "POST") {
    ...
    const sessionOnly = result.persisted === false || result.sessionOnly === true;
    let message = sessionOnly
      ? `Added ${result.name} (browser session — Netlify cannot write to GitHub)`
      : `Saved ${result.name} to disk`;
    if (result.buildWarning) {
      message = `Saved ${result.name} to disk, but rebuild warned: ${result.buildWarning}`;
    }
    return {
      status: 201,
      body: {
  ...
  if (pathname === "/tribes/history" && method === "GET") {
    try {
      const hist = await listEditHistory({
        tribeId: query.tribeId || query.id,
        limit: query.limit ? Number(query.limit) : 40,
      });
      return { status: 200, body: { ok: true, ...hist, writable: persist } };
    } catch (e) {
      return { status: 500, body: { ok: false, error: e?.message || String(e) } };
    }
  }

  const historyMatch = pathname.match(/^\/tribes\/([^/]+)\/history$/);
  if (historyMatch && method === "GET") {
    ...
    try {
      ...
      const samples = {};
      for (const ref of ["inf_t1", "inf_t2", "inf_t3", "cav_t1", "cav_t2", "cav_t3"]) {
        samples[ref] = await slotHistorySamples(id, ref, 8);
      }
      return {
        status: 200,
        body: { ok: true, tribeId: id, ...hist, samples, writable: persist },
      };
    } catch (e) {
      return { status: 500, body: { ok: false, error: e?.message || String(e) } };
    }
  ...
  const tribeIdMatch = pathname.match(/^\/tribes\/([^/]+)$/);
  if (tribeIdMatch && method === "PUT") {
    const id = decodeURIComponent(tribeIdMatch[1]);
    if (isCoreTribe(id)) {
      return {
        status: 403,
        body: { ok: false, error: `Cannot edit core tribe "${id}"` },
      };
    }
    const body = /** @type {Record<string, unknown>} */ (req.body || {});
    try {
      const result = await updateTribe(id, {
        name: body.name,
        theme: body.theme,
        type: body.type === "npc" ? "npc" : body.type === "playable" ? "playable" : undefined,
        palette: body.palette,
        historicalContext: body.historicalContext || body.context,
        era: body.era,
        region: body.region,
      ...
      let message = result.message || `Updated ${result.name}`;
      if (result.buildWarning) {
        message = `Updated ${result.name}, but rebuild warned: ${result.buildWarning}`;
      }
      return {
        status: 200,
        body: {
          ...
          persisted: result.persisted !== false,
          sessionOnly: result.sessionOnly === true || result.persisted === false,
          message,
        },
      };
    } catch (e) {
      const msg = e?.message || String(e);
      const status = /not found|not registered/i.test(msg) ? 404 : /core tribe|cannot edit/i.test(msg) ? 403 : 400;
      return { status, body: { ok: false, error: msg } };
    }
  }

  if (tribeIdMatch && method === "DELETE") {
    const id = decodeURIComponent(tribeIdMatch[1]);
    if (!persist) {
      return {
        status: 200,
