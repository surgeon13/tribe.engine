...
import { quantizeCost, quantizeStats } from "../balance/quantize.js";
import {
  CULTURE_LEXICONS,
  LEXICON_PRIORITY,
  matchCultureLexicon,
...
function scaleUtilityTroop(ref, base, scale) {
  ...
  return { stats: quantizeStats(stats), cost: quantizeCost(cost), cropUpkeep: base.cropUpkeep };
}
 ...
 * @param {string} roleId the cav_t3 role, which decides how it grows
 */
function applyCenterpiece(troops, handAuthored, roleId) {
  ...
  const { stats, factor } = liftToCenterpiece(
    horse.stats,
    rival,
    CENTERPIECE_LEAD,
    CENTERPIECE_AXIS[roleId] ?? "both"
  );
...
export function buildCustomProfile(opts) {
  ...
  applyCenterpiece(troops, Boolean(opts.troopOverrides?.cav_t3), slotRoles.cav_t3);
