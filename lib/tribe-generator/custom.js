...
/**
 * Score a flavor pack against lore + tribe name.
 * Name hits weigh more so "Arabs" prefers the arab pack over a generic desert mention in lore.
...
/** More specific culture packs beat generic biome packs on equal scores. */
const FLAVOR_PACK_PRIORITY = {
  israelite: 3,
  arab: 3,
  undead: 2,
  desert: 1,
...
function betterFlavorPack(candidate, candMeta, current, curMeta) {
  ...
  return (FLAVOR_PACK_PRIORITY[candidate.id] || 0) > (FLAVOR_PACK_PRIORITY[current.id] || 0);
}
...
const FLAVOR_PACKS = [
  ...
  {
    id: "desert",
    // Biome / Maghreb cues only — Arab peoples use the dedicated "arab" pack below.
    match: [
      "desert",
      "sand",
      "camel",
  ...
  },
  {
    ...
    era: "Arabian / early Islamic frontier",
    region: "Arabia & desert marches",
    logos: {
  ...
  {
    id: "israelite",
    // Include "israel" for names like "Kingdom of Israel"; lore-only "enemy of Israel" loses to name hits on other packs.
    ...
    region: "Levant",
    units: {
      // "Levy" is intentional here — biblical / Iron Age muster language.
      inf_t1: "Levy Spearman",
      inf_t2: "Slinger",
      inf_t3: "Gibbor",
      scout: "Watchman",
      cav_t1: "Chariot Runner",
      cav_t2: "Royal Chariot",
      cav_t3: "Horse Guard",
      ram: "Siege Ram",
...
function personalizePackUnits(units, label) {
  ...
  if (out.hero) {
    const bare = out.hero.replace(/\s+Hero$/i, "").trim();
    // Drop generic culture adjectives ("Arab Hero", "Desert Hero") so Ishmaelites ≠ "Ishmaelite Arab Hero"
    if (
      !bare ||
      /^(arab|desert|israelite|imperial|frost|tribal|sea|forest|mountain|steppe|undead)$/i.test(bare)
    ) {
      out.hero = `${label} Hero`;
    } else if (!out.hero.toLowerCase().includes(label.toLowerCase())) {
      out.hero = `${label} ${bare}`.replace(/\s+/g, " ").trim();
