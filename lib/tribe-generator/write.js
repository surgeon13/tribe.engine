...
import { appendEditHistory, snapshotFromTribeDoc } from "./edit-history.js";
...
export async function updateTribe(id, opts = {}) {
  ...
  if (!persist) {
    ...
    if (!displayName) throw new Error("Tribe name is required");
    const beforeSnap = opts.beforeSnapshot || null;
    ...
    const afterSnap = snapshotFromTribeDoc(result.dashboardTribe);
    const hist = await appendEditHistory({
      tribeId,
  ...
  const beforeSnap = snapshotFromTribeDoc(tribeDoc);
  ...
  /** @type {Record<string, { building: string, timeSeconds: number }> | null} */
  let trainingPatch = opts.training && typeof opts.training === "object" ? { ...opts.training } : null;
  ...
  for (const troop of tribeDoc.troops || []) {
    ...
    if (patch.role) ov.role = String(patch.role);
    ...
    if (patch.training && typeof patch.training === "object") {
      if (!trainingPatch) trainingPatch = {};
      trainingPatch[ref] = {
        building: String(patch.training.building || "barracks"),
        timeSeconds: clampInt(patch.training.timeSeconds ?? 0, 0, 1_000_000),
      };
      ov.training = { ...trainingPatch[ref] };
    }
  }
  ...
  let trainingByRef = null;
  if (trainingPatch) {
    const training = await readJson("tribe-training.json");
    const prev = training.tribes?.[tribeId] || {};
    trainingByRef = { ...prev };
    for (const [ref, patch] of Object.entries(trainingPatch)) {
      trainingByRef[ref] = {
        building: String(patch.building || prev[ref]?.building || "barracks"),
        timeSeconds: clampInt(patch.timeSeconds ?? prev[ref]?.timeSeconds ?? 0, 0, 1_000_000),
      };
    }
    training.tribes[tribeId] = trainingByRef;
    await writeJson("tribe-training.json", training);
  }
  ...
  const dashboardTribe = await resolveDashboardTribe(tribeDoc, type, trainingByRef || undefined);
  const afterSnap = snapshotFromTribeDoc(tribeDoc);
